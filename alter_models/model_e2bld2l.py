...
class Encoder(nn.Module):
    def __init__(self, vocab, embeding_dim, encoder_hidden_dim, decoder_hidden_dim, dropout):
        ...
        self.rnn = nn.LSTM(embeding_dim, encoder_hidden_dim, bidirectional = True)
    ...
    def forward(self, text, text_len):
        ...
        outputs, (hidden, _) = self.rnn(outputs)
...
class Decoder(nn.Module):
    ...
    def forward(self, input_, hidden, encoder_outputs, mask):
        ...
        output, (hidden, _) = self.rnn(rnn_input_, (hidden.unsqueeze(0), hidden.unsqueeze(0)))

        # second layer
        output = self.fc2(output)
        output, (hidden, _) = self.rnn(output)
